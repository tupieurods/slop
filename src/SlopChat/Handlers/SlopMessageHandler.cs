...
namespace SlopChat.Handlers
{
    public class SlopMessageHandler
    {
        ...
        public async Task HandleAsync(ITelegramBotClient bot, Message message, string userText, CancellationToken ct)
        {
            ...
            List<ChatMessage> history = _conversationManager.GetSnapshot(chatId);

            try
            {
                string response = await _openRouter.GetCompletionAsync(history, _options.DefaultModel, ct);
            ...
            catch(Exception ex)
            {
                ...
                await bot.SendMessage(chatId, "Something went wrong while getting a response.", replyParameters: new ReplyParameters { MessageId = message.MessageId }, cancellationToken: ct);
            }
        }
